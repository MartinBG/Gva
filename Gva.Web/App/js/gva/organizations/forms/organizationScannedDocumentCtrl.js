...
(function (angular, Select2, _) {
  ...
  function OrganizationScannedDocCtrl(
    $scope,
    $state,
    $stateParams,
    $compile,
    ...
    $scope.viewApplication = function (partIndex) {
      $state.go('root.organizations.view.documentApplications.edit', {
        id: $stateParams.id,
        ind: partIndex
      });
    };
  ...
  angular.module('gva').controller('OrganizationScannedDocCtrl', OrganizationScannedDocCtrl);
}(angular, Select2, _));
