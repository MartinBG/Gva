...
(function (angular, Select2, _) {
  ...
  function AirportScannedDocCtrl($scope, $state, $stateParams, $compile, AirportApplication) {
    ...
    $scope.appSelectOpt = {
      ...
      query: function (query) {
        AirportApplication.query({ id: $stateParams.id, term: query.term }).$promise
    ...
    $scope.viewApplication = function (partIndex) {
      $state.go('root.airports.view.documentApplications.edit', {
        id: $stateParams.id,
        ind: partIndex
      });
    };
  ...
  angular.module('gva').controller('AirportScannedDocCtrl', AirportScannedDocCtrl);
}(angular, Select2, _));
