    ...
    public class Atcl1Licence : IDataGenerator
    {
            ...
            IEnumerable<PartVersion<PersonRatingEditionDO>> ratingEditions)
        {
            ...
            foreach (var edition in ratingEditions)
            {
                ...
                if (rating.Content.AuthorizationId.HasValue)
                {
                    ...
                    {
                        string name = otherEndorsementCodes.Where(oe => authorization.Code.Contains(oe)).First();
            ...
            bool includeOtherEndorsements)
        {
            ...
            foreach (var edition in editions)
            {
                ...
                var locationIndicator = rating.Content.LocationIndicatorId.HasValue ? this.nomRepository.GetNomValue("locationIndicators", rating.Content.LocationIndicatorId.Value).Code : null;
                object result = null;
                if (withIssueDate)
                {
                    result = new
                    {
                        ICAO = locationIndicator,
                        SECTOR = sector,
