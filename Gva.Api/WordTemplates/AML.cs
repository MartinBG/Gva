    ...
    public class AML : IDataGenerator
    {
        ...
        private object[] GetAircrafts(IEnumerable<PersonRatingDO> includedRatings)
        {
            return includedRatings
                .Where(r => r.AircraftTypeGroup != null && r.Editions.Last().Limitations != null && r.Editions.Last().Limitations.Count == 0)
                .Select(r => new
                {
            ...
            return includedRatings
                ...
                .Select(r =>
                {
                    ...
                    return new
                    {
                        ...
                        LIMIT = (lastEdition.Limitations != null && lastEdition.Limitations.Count > 0) ?
                ...
                .Select(r =>
                    {
                        ...
                        return new
                        {
                            ...
                            CAT = r.AircraftTypeCategory.Code,
                            LIM = (lastEdition.Limitations != null && lastEdition.Limitations.Count > 0) ?
                            string.Join(",", lastEdition.Limitations.Select(l => l.Name)) :
                            "NP"
                        };
                    }).ToArray<object>();
