    ...
    public class Pilot142 : IDataGenerator
    {
        ...
        private List<object> GetInstructorData(IEnumerable<PartVersion<PersonRatingDO>> inspectorRatings, IEnumerable<PartVersion<PersonRatingEditionDO>> inspectorRatingEditions)
        {
            ...
            var result = inspectorRatings
                .Where(p => p.Content.Authorization != null && p.Content.Authorization.Code != "RTO" && p.Content.Authorization.ParentValueId == authorizationGroupId)
                .Select(p =>
                    {
        ...
        private List<object> GetExaminerData(IEnumerable<PartVersion<PersonRatingDO>> inspectorRatings, IEnumerable<PartVersion<PersonRatingEditionDO>> inspectorRatingEditions)
        {
            var authorizationGroupId = this.nomRepository.GetNomValues("authorizationGroups")
                .First(nv => nv.Code == "FC")
                .NomValueId;
