...
{
    public class Convoy : IDataGenerator
    {
        ...
        public JObject GetData(int lotId, string path, int index)
        {
            ...
            IEnumerable<JObject> ratings = this.GetRatings(includedRatings);
            var TRatings = ratings.Select(r =>
            {
            ...
            dynamic documents1 = null;
            dynamic documents2 = null;
            if (documents.Length > 0)
            { 
                documents1 = documents.Take(documents.Length / 2);
                documents2 = documents.Skip(documents.Length / 2);
            }
            ...
            var json = new
            {
                root = new
                {
                    ...
                    L_LICENCE_TYPE_NAME = licenceType.Name.ToUpper().ToLower(),
                    L_LICENCE_TYPE_NAME_TRANS = licenceType.NameAlt == null ? string.Empty : licenceType.NameAlt.ToLower(),
                    L_LICENCE_NO = licenceNumber,
                    L_LICENCE_HOLDER = this.GetPersonData(personData, personAddress),
                    L_LICENCE_PRIV = this.GetLicencePrivilege(edition),
                    ...
                    T_ACTION = edition.Get<string>("licenceAction.name"),
                    T_ISSUE_DATE = edition.Get<DateTime>("documentDateValidFrom"),
                    T_DOCUMENTS = documents1,
                    T_DOCUMENTS2 = documents2,
        ...
        private IEnumerable<JObject> GetRatings(IEnumerable<JObject> includedRatings)
        {
        ...
        private object GetLicencePrivilege(JObject edition)
        {
